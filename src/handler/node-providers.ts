  ...
  try {
    ...
    if (axios.isAxiosError(error)) {
      ...
      if (error.code === "ECONNABORTED" || error.code === "ETIMEDOUT") {
        errorMessage =
          "⌛ The request timed out after multiple retries. The IC API might be experiencing high load.";
        ...
        errorMessage =
          "🔍 Node provider data not found. The API may have changed.";
      } else if (statusCode === 422) {
        errorMessage = "⚠️ Invalid request parameters.";
      } else if (statusCode >= 500) {
        errorMessage =
          "⚠️ IC API is currently unavailable. Please try again later.";
      }
    }
