  ...
  try {
    ...
    const status: NetworkStatus = {
      ...
      nodes: {
        ...
        down:
          (nodesRes.data?.total_nodes?.[0]?.[1] || 0) -
          (nodesRes.data?.up_nodes?.[0]?.[1] || 0),
      },
    ...
    const message = [
      ...
      `- Nodes: ${status.nodes.total.toLocaleString()} (▲ ${
        status.nodes.up
      } | ▼ ${status.nodes.down})`,
    ...
    const errorMessage =
      "❌ Failed to fetch network status. The IC API may be temporarily unavailable.";
